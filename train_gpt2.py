...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        assert len(sd_keys) == len(
            sd_keys_hf
        ), f"mismatched key length: {len(sd_keys)} != {len(sd_keys_hf)}"
        for k in sd_keys_hf:
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        # only decay 2D+, but not 1D
        # e.g. all weight tensors in matmuls + embeddings decay, all biases and layernorms don't
...
class DataLoaderLite:
    def __init__(self, B, T, proc_rank, num_procs, split):
        ...
        if master_process:
            print(f"found {len(shards)} shards fpr split {split}")
        self.reset()
...
if ddp:
    ...
    master_process = ddp_rank == 0  # use RNAK=0 to do logging, checkpointing etc
else:
    # vanilla, non-DDP
    ddp_rank = 0
    ddp_local_rank = 0
...
train_loader = DataLoaderLite(
    B=B, T=T, proc_rank=ddp_rank, num_procs=ddp_world_size, split="train"
)
val_loader = DataLoaderLite(
    B=B, T=T, proc_rank=ddp_rank, num_procs=ddp_world_size, split="val"
)
...
for step in range(max_steps):
    ...
    for micro_step in range(grad_accum_steps):
        ...
        with torch.autocast(device_type=device, dtype=torch.bfloat16):
            logits, loss = model(x, y)
        loss = loss / grad_accum_steps  # since cross_entropy is doing mean
